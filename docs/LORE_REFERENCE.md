...
The chamber should feel like refuge first. It can be quiet, useful, and safe by default. As the player moves through later Chamber states, the sky and boundaries can show that privacy eroding: vascular and neural structures gaining stranger colors, Qliphoth influence disturbing the pulse, and Silent Archon restraint muting the chamber. Apotheos remains reserved design space here: its Chamber theme id exists, but the current visual treatment is deliberately black and blank-slate pending a later fungal transformation pass.
