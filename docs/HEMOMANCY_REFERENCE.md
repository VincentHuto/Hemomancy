...
| **Harbinger Voyager** | ![](../src/main/resources/assets/hemomancy/textures/entity/harbinger_voyager/harbinger_voyager.png) | Creature | Future active-vessel captain-scholar NPC; dialogue-only research leader for reef, vent, and wreck survey expeditions. Always placed by active-vessel spawn helper when that structure is implemented. |
| **Harbinger Votary Wayfarer** | ![](../src/main/resources/assets/hemomancy/textures/entity/harbinger_votary_wayfarer/harbinger_votary_wayfarer.png) | Creature | Future active-vessel junior Votary NPC with a 1-in-5 companion spawn rule. Dialogue-only observer learning from the Voyager; no trades, quests, rewards, or ordinary spawning. |
...
Mobs are tagged by tendency: `FUNGAL_TAG`, `UMBRAL_TAG`, `INCANDESCENT_TAG`, `FERRIC_TAG`, `VIVACIOUS_TAG`, `RUINOUS_TAG`, `NEUROTIC_TAG`, `FERVENT_TAG`, `FRIGID_TAG`. Chalybeate Snails are Ferric-aligned and included in `specimen_jar_capturable`; Blood Lantern Jellies are Vivacious-aligned and also specimen-jar capturable.
...
| **Erythrocoral Reef** | `erythrocoral_reef` | 0.95 | Rain | Rare warm/lukewarm deep-ocean biome injected through `ErythrocoralReefRegion`; murky red-violet water, light crimson spores, Erythrocoral reef clusters, Blood Lantern Jelly, Barbed Urchins, tropical fish, pufferfish, squid, and rare dolphins |
...
| Erythrocoral Reef Feature | Code-generated warm-ocean reef cluster feature using Erythrocoral blocks plus restrained Hemomancy accents; validates submerged stable floors to avoid floating coral |
...
- **Erythrocoral Reef V1** - **Implemented:** `erythrocoral_reef` is a true TerraBlender-injected warm deep-ocean biome with red-violet water/fog, light crimson spore ambience, stable-floor procedural reef clusters, the Erythrocoral block family, shears-first fragment harvesting, Vivacious Spores support recipes, and the non-hostile `blood_lantern_jelly` ambient mob. It is now the first target biome for Harbinger Voyager Wrecks and remains tagged in `#hemomancy:harbinger_voyager_vessel_candidates` for future active expedition logic.
- **Harbinger Voyager Wrecks V1** - **Implemented:** `harbinger_voyager_wreck` is a rare sunken research-vessel structure targeted at Erythrocoral Reefs through `#hemomancy:has_structure/harbinger_voyager_wreck`. It uses three compact NBT variants, modest salvage loot, discovery inscriptions (`red_current_chart`, `last_covenant_watch`, `vent_survey_fragment`), the lore-only `salt_stained_voyager_log`, and the structure-only `brined_votary` remnant. Active Harbinger voyager expeditions remain future neutral/tense encounter hooks.
- **Active Harbinger Voyager NPCs** - **Implemented foundation:** `harbinger_voyager` and `harbinger_votary_wayfarer` are registered dialogue-only NPCs with spawn eggs, attributes, render hooks, textures, and item inquiry data. `ActiveHarbingerVoyagerNpcSpawner` always places one captain-scholar Voyager for future active-vessel structures and uses `ActiveHarbingerVoyagerNpcRules` for the 1-in-5 Votary Wayfarer companion rule. Active ship structures, trades, faction tension, and sailing behavior remain future work.
