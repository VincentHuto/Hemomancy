...
> **Release scope:** Saints are intentionally modular expansion content. Initial lore, the shared sarcophagus foundation, and Hemorath's complete trial are sufficient for the first release. Bespoke rooms, environmental storytelling, tailored victory-condition refinement, placement tuning, models/animations, and final balance for Seraphae, Putriciel, and Velorum are reserved for the first major post-release Saints update, not tracked as launch blockers.
...
| Lethean / pale crafting recipes | `Implemented` | Tears of Silthmere, Lethean Poppy Wreath, Pale Distillate, Pale Silver, Cleansed Stone, Pallid Lantern, Pallid Silver Chain, Pale Silver Bars, Pale Silver Bells, Pale Field Ink, Lethean Chalice, Pallid Icon, Unstained armor/tools, Pallid Retort distillation, and White Humor purification recipes are present under `data/hemomancy/recipe/`. Ghost Pipe also feeds the Stillwater Condenser. The rejected Pale Silver Pickaxe and Verdigris Censer remain unregistered. |
| Unstained Observances | `Implemented` | Five Acolyte dialogue assignments persist and sync through `UnstainedProgress`. `UnstainedObservanceHelper` validates offerings and one-time rewards; the Book of Observances opens the progress ledger, which displays accepted/completed work and the next rite. |
