...
Craft with five Bloodwood Planks and four Hematic Iron Scraps. The Alchemist inquiry explains identity, controls, glazing and dismantling. Runtime models reuse authored Bloodwood/Hematic Iron textures, with an editable `.bbmodel` and deterministic exporter under `tools/model_export/phlebotomists_cabinet.py`. No rank, research reward or creature rendering was added. See [player guide](../wiki/Phlebotomists-Cabinet.md) and [validation checklist](../wiki/Phlebotomists-Cabinet-Validation.md).
...
**Phlegethontic Bombardier appearance:** the reference-matched repaint uses charcoal-brown chitin, rusty-red wing markings and abdominal joints, and worn bone rims and tips. Its 128×128 PNG matches the authored 128×128 UV canvas at native 16x texel density. The Java model matches the latest Blockbench export's 63 cuboids, including the second eye with mirrored geometry and UVs. The runtime atlas and embedded Blockbench texture are synchronized; animation and gameplay are unchanged.
