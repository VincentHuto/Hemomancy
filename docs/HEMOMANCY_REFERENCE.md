...
After the first projection, choosing Silence records `SILENT_PENDING`. Successful Pruning changes the owned Bloom from living to severed/portal-open rather than removing it. Only its owner can enter the separate 50x50 barrier-floor arena in the Chamber dimension. The fight spawns one owned Crowned Refusal, transfers the ordeal identity into the Evening Star phase, and prevents duplicate Vespers on retry. Death, logout, manual exit, or restart clears the active encounter but leaves the portal open. Verified phase-two victory records `SILENT_ARCHON`, grants Memory of Vesper exactly once (with a persistent delivery fallback), changes the Chamber to the Silent theme, and seals the severed Bloom as an inert trophy. Creative players may remove the trophy for administration.
...
|---|---|---|---|
| Silent / Refusal Archon | **Vesper** | `hemomancy:vesper_crowned_refusal` -> `hemomancy:vesper_evening_star` | `Implemented`: entered through Pruning and the owner-bound Severed Qliphoth; retry, victory, reward, seal, and Silent Archon state are wired. |
