...
- **~~Annetta Knowles / Stained Priestess~~** — **Encounter fully wired.** `AnnettaKnowlesEntity`, `StainedPriestessEntity`, and `LatentAnnettaInfectionEntity` are all registered and functional. `BrokenChurchStructure` spawns Annetta in COWERING state with contextual scene dressing. Harbinger route (ToothPecks Jar) triggers Phase 1 → Phase 2 (Stained Priestess) boss flow with route-gated drops. Unstained route (Draught of Still Mercy + Clarity unlocked) cures Annetta into CURED_SUPPORT and spawns the Latent Infection as a separate fight. Dialogue trees (`AnnettaDialogueTrees`) cover cowering, cured-support, and resolved states. **Still WIP:** dedicated entity model, texture, and GeckoLib animations; Annetta's teeth/nails/hair combat identity for Phase 1; SanguisLancea item rendering. See §19.3 and LORE_REFERENCE §11.
