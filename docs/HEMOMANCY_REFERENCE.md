...
The Degree 8 / Apotheos slot now starts its dedicated Chamber treatment with an **APOTHEOS floor funnel**. It still selects `hemomancy:apotheos` and keeps tier 3 room growth/debug override support, but the theme uses a dedicated `ApotheosChamberEffects` strategy instead of the previous empty renderer. This first pass is renderer-only: a black/red procedural floor aperture below the refuge, built from a skybox-space annular funnel mesh and shader-driven concentric bands that expand outward from a dark center, suggesting the player has ascended through the Qliphoth black-hole endpoint. The APOTHEOS walls and ceiling remain pending separate passes; for now, the theme keeps the surrounding sky black and disables the shared vascular/neural overlay layers, clouds, nebula, and membrane pulse.
