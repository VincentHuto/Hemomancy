...
The Degree 8 / Apotheos slot is intentionally a blank-slate Chamber theme for now. It still selects `hemomancy:apotheos` and keeps tier 3 room growth/debug override support, but renders a completely black sky with no textured skybox, clouds, nebula, vascular/neural layers, membrane pulse, or custom renderer until the Apotheos Chamber treatment is redesigned.
