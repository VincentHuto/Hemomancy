...
At Degree 2, the Harbinger Alchemist introduces one main Votary assignment and two optional side assignments. The Main D2 assignment, **The First Separation**, focuses on the Vial Centrifuge loop: obtain a Vial Centrifuge, carry a sampled Blood Vial, successfully start a valid centrifuge separation (`first_separation_started`), and recover any enzyme. This teaches blood sampling and separation without requiring full enzyme completion.
...
The Mnemonic Lowtide progression slot is the first transitional disturbance between the default refuge state and later Archon/Qliphoth branches. It keeps the sky restrained, but renders a nearby but unreachable glossy black, red, and parchment-beige skybox-space horizon tide below the blood-wood refuge. The lake is renderer-only: it adds no fluid, collision, hazard, or movement rule, ignores world-distance fog so the black/red/parchment liquid remains legible at Chamber heights, and moves with the Chamber sky rather than existing beneath the platform, so it reads as endless blood-memory pressing up around the player's private room rather than a fallable ocean.
...
- **Armor-born active abilities:** Full final Bloodlust sets register armor set abilities that appear as a third inner wedge in the manipulation radial menu beside Blood Absorption and Blood Projection. The wedge is hidden unless the complete four-piece set is worn, displays the set helmet as its icon, and sends only the ability id to the server. Server activation rechecks the full set, cooldown, blood cost, and ability-specific validation before applying any effect.
...
- **Phantasmal Step:** Full Phantasmal Bloodlust unlocks `hemomancy:phantasmal_step`, an unrestricted line-of-sight teleport modeled after Umbral Step but without darkness restrictions or cooldown. When struck, the set can blind and outline the attacker, then displace them roughly eight blocks away if a safe landing position exists.
- **Stats:** Defense 3/6/8/3 (20 total), Toughness 3.0, KB Resist 0.1, Durability Ã—37, Enchantability 15
- **Repair:** Hematic Iron Scrap
