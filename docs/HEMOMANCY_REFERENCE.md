...
- **Materials** â€” panning/zoomable atlas catalogue of mod items and blocks. Harbinger and Unstained share the atlas controller, but use path-specific cosmetic buckets for category labels, color, and local grouping. Vein traces follow explicit material parent links rather than bucket membership. Locked future materials stay hidden; only the immediate next tier appears as next-tier veiled material nodes with requirement text and without revealing names, icons, recipes, or descriptions.
