...
public class DrudgeHarvestCropGoal extends RemoveBlockGoal {
	...
	public void tick() {
		...
		if ((blockpos1 != null)) {
			if (HLBlockUtils.isCrop(world, blockpos1)) {
				CropBlock crop = (CropBlock) world.getBlockState(blockpos1).getBlock();
				if (!(crop instanceof BeetrootBlock)) {
					if (HLBlockUtils.isCropFullyGrown(world, blockpos1)) {
					...
					if (HLBlockUtils.isBeetFullyGrown(world, blockpos1)) {
						if (this.breakingTime > 0) {
							Vec3 vector3d = this.entity.getDeltaMovement();
							this.entity.setDeltaMovement(vector3d.x, 0.3D, vector3d.z);
							if (!world.isClientSide) {
	...
	protected boolean isValidTarget(LevelReader worldIn, BlockPos pos) {
		...
		if (this.mob instanceof EntityDrudge) {
			if (HLBlockUtils.isCrop(worldIn, pos)) {
				CropBlock crop = (CropBlock) worldIn.getBlockState(pos).getBlock();
				// This is because APPERENTLY beets have their own growth property
				if (!(crop instanceof BeetrootBlock)) {
					return HLBlockUtils.isCropFullyGrown(worldIn, pos);
				} else {
					return HLBlockUtils.isBeetFullyGrown(worldIn, pos);
				}
	...
	public void playDestroyProgressSound(Level worldIn, BlockPos pos) {
