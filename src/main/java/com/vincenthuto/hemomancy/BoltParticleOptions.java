...
public class BoltParticleOptions {
	...
	public BoltParticleOptions(BoltRenderInfo info, Vec3 start, Vec3 end, int segments, int count, float size,
	...
	public BoltParticleOptions count(int count) {
		return new BoltParticleOptions(this.renderInfo, this.start, this.end, this.segments, count, this.size,
	...
	public BoltParticleOptions size(float size) {
		return new BoltParticleOptions(this.renderInfo, this.start, this.end, this.segments, this.count, size,
				this.lifespan, this.spawnFunction, this.fadeFunction);
	}
	...
	public BoltParticleOptions spawn(SpawnFunction spawnFunction) {
		return new BoltParticleOptions(this.renderInfo, this.start, this.end, this.segments, this.count, this.size,
	...
	/**
	 * Define the lifespan (in ticks) of this bolt, at the end of which the bolt
	 * will expire.
	 *
	...
	public List<BoltQuads> generate() {
		...
		for (int i = 0; i < count; i++) {
			...
			while (!drawQueue.isEmpty()) {
				...
				float progress = data.progress() + (1F / segments)
						* (1 - renderInfo.parallelNoise + random.nextFloat() * renderInfo.parallelNoise * 2);
				Vec3 segmentEnd;
				...
				if (progress >= 1 && segmentDiffScale <= 0) {
					...
					perpendicularDist = renderInfo.segmentSpreader.getSegmentAdd(perpendicularDist, randVec, maxDiff,
							segmentDiffScale, progress, rand);
					// new vector is original + current progress through segments + perpendicular
					// change
				...
				if (progress >= 1) {
					...
					drawQueue.add(
							new BoltInstructions(segmentEnd, progress, perpendicularDist, quadData.getRight(), false));
				} else if (random.nextFloat() < renderInfo.branchContinuationFactor) {
					// branch continuation
					drawQueue.add(
							new BoltInstructions(segmentEnd, progress, perpendicularDist, quadData.getRight(), true));
				}
	...
	private record BoltInstructions(Vec3 start, float progress, Vec3 perpendicularDist, @Nullable QuadCache cache,
			boolean isBranch) {
	}
	...
	/**
	 * A SpreadFunction defines how far bolt segments can stray from the
	 * straight-line vector, based on parallel 'progress' from start to finish.
	 *
	 * @author aidancbrady
	 */
	public interface SpreadFunction {
		...
		/**
		 * A steady linear increase in perpendicular noise, followed by a steady
	...
	/**
	 * A RandomFunction defines the behavior of the RNG used in various bolt
	...
	/**
	 * A SegmentSpreader defines how successive bolt segments are arranged in the
	 * bolt generation calculation, based on previous state.
	 *
	 * @author aidancbrady
	 */
	public interface SegmentSpreader {
		...
		/**
		 * Don't remember where the last segment left off, just randomly move from the
		 * straight-line vector.
		 */
		SegmentSpreader NO_MEMORY = (perpendicularDist, randVec, maxDiff, scale, progress, rand) -> randVec
				.scale(maxDiff * rand);

		/**
		 * Move from where the previous segment ended by a certain memory factor. Higher
		 * memory will restrict perpendicular movement.
		 */
	 ...
	 * A spawn function generates a lower and upper bound on a spawn delay (via
	 * getSpawnDelayBounds()), for which an intermediate value is chosen randomly
	 * from a uniform distribution (getSpawnDelay()). Spawn functions can also be
	 * defined as 'consecutive,' in which cases the Bolt Renderer will always begin
	...
	public interface SpawnFunction {
		...
		/**
		 * Spawns bolts with a specified delay and specified noise value, which will be
		 * randomly applied at either end of the delay bounds.
		 */
	...
	/**
	 * A bolt's fade function allows one to define lower and upper bounds on the
	 * bolt segments rendered based on lifespan. This allows for dynamic 'fade-in'
	...
	public interface FadeFunction {
		...
		/**
		 * Render bolts with a segment-by-segment 'fade' in and out, with a specified
		 * fade duration (applied to start and finish).
		 */
	...
	public static class BoltRenderInfo {
		...
		/**
		 * How much variance is allowed perpendicular to the straight line vector.
		 * Scaled by distance and spread function.
		 */
		...
		private BoltRenderInfo(float parallelNoise, float spreadFactor, float branchInitiationFactor,
				float branchContinuationFactor, Vector4f color, RandomFunction randomFunction,
				SpreadFunction spreadFunction, SegmentSpreader segmentSpreader) {
		...
		private static BoltRenderInfo defaultConfig() {
			return new BoltRenderInfo(0.1F, 0.1F, 0.0F, 0.0F, new Vector4f(0.0F, 0.7764F, 1F, 0.8F),
					RandomFunction.GAUSSIAN, SpreadFunction.SINE, SegmentSpreader.NO_MEMORY);
		}

		public BoltRenderInfo noise(float parallelNoise, float spreadFactor) {
			return new BoltRenderInfo(parallelNoise, spreadFactor, this.branchInitiationFactor,
					this.branchContinuationFactor, this.color, this.randomFunction, this.spreadFunction,
					this.segmentSpreader);
		}

		public BoltRenderInfo branching(float branchInitiationFactor, float branchContinuationFactor) {
			return new BoltRenderInfo(this.parallelNoise, this.spreadFactor, branchInitiationFactor,
					branchContinuationFactor, this.color, this.randomFunction, this.spreadFunction,
					this.segmentSpreader);
		}
		...
		public BoltRenderInfo spreadFunction(SpreadFunction spreadFunction) {
			return new BoltRenderInfo(this.parallelNoise, this.spreadFactor, this.branchInitiationFactor,
					this.branchContinuationFactor, this.color, this.randomFunction, spreadFunction,
					this.segmentSpreader);
		}

		public BoltRenderInfo color(Vector4f color) {
			return new BoltRenderInfo(this.parallelNoise, this.spreadFactor, this.branchInitiationFactor,
					this.branchContinuationFactor, color, this.randomFunction, this.spreadFunction,
					this.segmentSpreader);
		}
	}
}