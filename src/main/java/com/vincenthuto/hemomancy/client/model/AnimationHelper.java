...
public final class AnimationHelper {
	...
	/**
	 * Animate the given {@link IAnimatedModel} based on the given
	 * {@link AnimationState} of the given {@link AnimationDefinition} at the given
	 * time with the given speed
	 * 
	 ...
	 */
	public static void animate(IAnimatedModel<?> model, AnimationState state, AnimationDefinition definition,
			float time) {
	...
	public static void animate(IAnimatedModel<?> model, AnimationState state, AnimationDefinition definition,
		...
		state.ifStarted(
				animState -> animate(model, definition, animState.getAccumulatedTime(), 1.0F, ANIMATION_VECTOR_CACHE));
	}

	/**
	 * Apply the initial state (time = 0) of the given {@link AnimationDefinition}
	 * to the given {@link IAnimatedModel}
	 * 
	 * @param model      The model being animated
	 ...
	 * @param scale           The scale to be applied to the outputs of the
	 *                        animation
	 * @param animVecCache    A cache vector to avoid allocation new vector
	 *                        instances for the animation outputs
	 */
	public static void animate(IAnimatedModel<?> model, AnimationDefinition definition, long accumulatedTime,
			float scale, Vector3f animVecCache) {
