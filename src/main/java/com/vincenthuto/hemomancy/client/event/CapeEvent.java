...
public final class CapeEvent {
	...
	public static CapeEvent instance() {
	...
	public static void renderLevelLast(RenderLevelStageEvent   event) {
		...
		float delta = HLClientUtils.getPartialTicks();
		if (!player.isInvisible() && !player.isFallFlying() && !player.isSleeping() && delta != 1) {
			RenderCape cape = getCape(player);
			if (cape.isPresent(player)) {
				Vector3d locVec = new Vector3d(
						player.getX() - cape.posX
								- mc.getBlockEntityRenderDispatcher().camera.getLookVector().x,
						player.getY() - cape.posY
								- mc.getBlockEntityRenderDispatcher().camera.getLookVector().y,
						player.getZ() - cape.posZ
								- mc.getBlockEntityRenderDispatcher().camera.getLookVector().z);
				cape.render(event.getPoseStack(), player, locVec.x, locVec.y, locVec.z, delta);
			}
		}
	...
	public static void onClientTick(TickEvent.ClientTickEvent event) {
		if (event.side == LogicalSide.CLIENT && event.phase == TickEvent.Phase.END) {
			Level world = HLClientUtils.getWorld();
			if (world != null) {
	...
	private static final class Holder {
		private static final CapeEvent INSTANCE = new CapeEvent();
	}
	...
	private static final class RenderCape {
		...
		void render(PoseStack matrixStack, Player player, double x, double y, double z, float delta) {
			...
			RenderSystem.setShaderTexture(0, fallBackCape);

			Matrix4f matrix = matrixStack.last().pose();
			matrixStack.translate((float) x - player.getLookAngle().x, (float) y - player.getLookAngle().y + 0.125,
		...
		private static final class PlayerCollisionResolver extends PlayerResolver {
			...
			public void resolve(Player player, Point point) {
				...
				if (player.isCrouching()) {
					...
					dy /= len;
					dz /= len;
					float rz = (point.posX - (float) player.getX()) * dx
							+ (point.posZ - (float) player.getX()) * dz;
