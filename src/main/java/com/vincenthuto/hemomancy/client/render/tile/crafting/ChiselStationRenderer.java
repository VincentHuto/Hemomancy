...
public class ChiselStationRenderer implements BlockEntityRenderer<ChiselStationBlockEntity> {
	...
	private final ChiselStationModel model;

	public ChiselStationRenderer(BlockEntityRendererProvider.Context context) {
		this.model = new ChiselStationModel(context.bakeLayer(ChiselStationModel.LAYER_LOCATION));
	}

	@Override
	public boolean shouldRenderOffScreen(ChiselStationBlockEntity te) {
		return true;
	}
	...
	private void renderStationModel(PoseStack poseStack, MultiBufferSource bufferIn,
									ChiselStationBlockEntity te, int combinedLightIn, int combinedOverlayIn) {
		poseStack.pushPose();
