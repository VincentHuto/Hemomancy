...
public class ScarPatternItemRenderer extends BlockEntityWithoutLevelRenderer {
	...
	private boolean renderPreparedScarIcons(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack,
		...
		for (int i = 0; i < scarIds.size() && i < QUADRANT_OFFSETS.length; i++) {
			ItemStack scarStack = scarStackFor(scarIds.get(i));
			...
			renderScarOverlay(scarStack, poseStack, buffer, combinedLight, combinedOverlay, mc, i);
			poseStack.popPose();
		}
		return true;
	}

	private void renderScarOverlay(ItemStack scarStack, PoseStack poseStack, MultiBufferSource buffer,
			int combinedLight, int combinedOverlay, Minecraft mc, int seedOffset) {
		BakedModel model = mc.getItemRenderer().getModel(scarStack, mc.level, null, 59 + seedOffset);
	...
	private static ItemStack scarStackFor(ResourceLocation scarId) {
		Item item = BuiltInRegistries.ITEM.get(scarId);
		if (item == Items.AIR) {
