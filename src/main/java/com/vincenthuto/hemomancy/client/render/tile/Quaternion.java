...
public final class Quaternion {
	...
	public Quaternion(com.vincenthuto.hutoslib.math.Quaternion pMatrix) {
		this.i = pMatrix.i();
		this.j = pMatrix.j();
		this.k = pMatrix.k();
		this.r = pMatrix.r();
	...
	public static Quaternion fromYXZ(float pY, float pX, float pZ) {
		...
		quaternion.mul(new Quaternion(0.0F, (float) Math.sin((double) (pY / 2.0F)), 0.0F,
				(float) Math.cos((double) (pY / 2.0F))));
		quaternion.mul(new Quaternion((float) Math.sin((double) (pX / 2.0F)), 0.0F, 0.0F,
				(float) Math.cos((double) (pX / 2.0F))));
		quaternion.mul(new Quaternion(0.0F, 0.0F, (float) Math.sin((double) (pZ / 2.0F)),
				(float) Math.cos((double) (pZ / 2.0F))));
		return quaternion;
	}

	public static Quaternion fromXYZDegrees(Vector3f pDegreesVector) {
		return fromXYZ((float) Math.toRadians((double) pDegreesVector.x()),
				(float) Math.toRadians((double) pDegreesVector.y()),
				(float) Math.toRadians((double) pDegreesVector.z()));
	}
	...
	public Vector3f toXYZ() {
		...
		return Math.abs(f5) > 0.999F * f4
				? new Vector3f(2.0F * (float) Math.atan2((double) this.i(), (double) this.r()), f6, 0.0F)
				: new Vector3f(
						(float) Math.atan2((double) (2.0F * this.j() * this.k() + 2.0F * this.i() * this.r()),
								(double) (f - f1 - f2 + f3)),
						f6, (float) Math.atan2((double) (2.0F * this.i() * this.j() + 2.0F * this.r() * this.k()),
								(double) (f + f1 - f2 - f3)));
	}

	public Vector3f toXYZDegrees() {
		Vector3f vector3f = this.toXYZ();
		return new Vector3f((float) Math.toDegrees((double) vector3f.x()),
				(float) Math.toDegrees((double) vector3f.y()), (float) Math.toDegrees((double) vector3f.z()));
	}
	...
	public Vector3f toYXZ() {
		...
		float f6 = (float) Math.asin((double) (f5 / f4));
		return Math.abs(f5) > 0.999F * f4
				? new Vector3f(f6, 2.0F * (float) Math.atan2((double) this.j(), (double) this.r()), 0.0F)
				: new Vector3f(f6,
						(float) Math.atan2((double) (2.0F * this.i() * this.k() + 2.0F * this.j() * this.r()),
								(double) (f - f1 - f2 + f3)),
						(float) Math.atan2((double) (2.0F * this.i() * this.j() + 2.0F * this.r() * this.k()),
								(double) (f - f1 + f2 - f3)));
	}
