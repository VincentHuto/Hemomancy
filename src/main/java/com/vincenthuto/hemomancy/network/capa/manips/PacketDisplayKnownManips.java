...
public class PacketDisplayKnownManips {
	...
	public static void handle(final PacketDisplayKnownManips message, final Supplier<NetworkEvent.Context> ctx) {
		ctx.get().enqueueWork(() -> {
			...
			player.displayClientMessage( Component.translatable("Selected: " + known.getSelectedManip().getProperName()),
					false);
			for (int i = 0; i < known.getKnownManips().size(); i++) {
				player.displayClientMessage(
						 Component.translatable("Manipulation " + i + ": " + known.getManipList().get(i).getProperName()),
						false);
			}
		});
