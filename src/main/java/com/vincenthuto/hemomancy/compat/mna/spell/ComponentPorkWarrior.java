...
public class ComponentPorkWarrior extends SpellEffect {
	...
	static {
		 required_reagents.add(new SpellReagent( null, new ItemStack(Items.COOKED_PORKCHOP),
		 false, false, true));
	}

	public ComponentPorkWarrior(ResourceLocation guiIcon) {
	...
	public ComponentApplicationResult ApplyEffect(SpellSource source, SpellTarget target,
		...
		if (!(target.getEntity() instanceof ItemEntity)
				|| ((ItemEntity) target.getEntity()).getItem().getItem() != Items.PORKCHOP) {
		...
		Mob spawn = (Mob) type.create(context.getLevel()); // double check that all the EntityTypes inherit from Mob;
															// these all do.
		if (spawn == null) {
			return ComponentApplicationResult.FAIL;
		}
