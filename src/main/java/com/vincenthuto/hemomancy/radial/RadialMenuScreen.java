...
public class RadialMenuScreen extends Screen
{
    ...
    public void tick()
    {
        ...
        else if (!RadialClientEvents.isKeyDown(RadialClientEvents.OPEN_TOOL_MENU_KEYBIND))
        {
    ...
    private boolean trySwap(int slotNumber, ItemStack itemMouseOver)
    {
        ...
        if (inHand.getCount() > 0 || itemMouseOver.getCount() > 0)
        {
            ...
            PacketHandler.CHANNELKNOWNMANIPS.sendToServer(new SwapItems(slotNumber));
        }
    ...
    {
        if (RadialClientEvents.isKeyDown(RadialClientEvents.CYCLE_TOOL_MENU_LEFT_KEYBIND))
        {
            if (!keyCycleBeforeL)
            {
                menu.cyclePrevious();
        ...
        if (RadialClientEvents.isKeyDown(RadialClientEvents.CYCLE_TOOL_MENU_RIGHT_KEYBIND))
        {
            if (!keyCycleBeforeR)
            {
                menu.cycleNext();
