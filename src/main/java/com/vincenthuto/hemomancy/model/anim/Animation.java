...
import com.vincenthuto.hemomancy.model.anim.Animation.Bone;
...
public record Animation(int lengthInTicks, float lengthScale, Map<String, Bone> bones) {
	...
	private static Vector3f lerp(Map.Entry<Float, Vector3f> a, Map.Entry<Float, Vector3f> b, float totalProgress,
			Easing easing) {
		...
		return new Vector3f(Mth.lerp(time, start.x(), end.x()), Mth.lerp(time, start.y(), end.y()),
				Mth.lerp(time, start.z(), end.z()));
	}
	...
	public void tick(Map<String, ModelPart> map, float ageInTicks) {
		...
		bones.forEach((name, bone) -> {
			...
			final Vector3f scales = getTransformForTime(bone.scales, progress, bone.easing);
	...
	public record Bone(Map<Float, Vector3f> rotations, Map<Float, Vector3f> translations, Map<Float, Vector3f> scales,
			Easing easing) {
		public static class Builder {
