			...
			if (volume.isActive()) {
				if (volume.getBloodVolume() > cost) {
					...
					player.displayClientMessage(
							new TextComponent("Not Enough Blood to be Shed!").withStyle(ChatFormatting.RED), true);
				}
			} else {
				player.displayClientMessage(
						new TextComponent("You strain your body but nothing happens!").withStyle(ChatFormatting.RED),
						true);
			}
		}
	}
