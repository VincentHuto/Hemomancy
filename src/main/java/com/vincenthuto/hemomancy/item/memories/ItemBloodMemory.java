...
public class ItemBloodMemory extends Item {
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		...
		if (getManip() != null) {
			tooltip.add( Component.translatable(getManip().getProperName()));
		}
	}
				...
				if (volume.isActive()) {
					if (!playerIn.isShiftKeyDown()) {
						if (!known.doesListContainName(knownList, getManip())) {
							...
							playerIn.displayClientMessage( Component.translatable("Player Already Knowns This Manipulation!")
									.withStyle(ChatFormatting.DARK_RED), true);
						}
					}
				} else {
					playerIn.displayClientMessage(
							 Component.translatable("You lack understanding of what your even holding...")
