...
public class ItemBloodGourd extends Item implements IRune {
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		...
		if (bloodPresent) {
			IBloodVolume bloodVolume = stack.getCapability(BloodVolumeProvider.VOLUME_CAPA)
					.orElseThrow(NullPointerException::new);
			tooltip.add(Component.translatable("Max Blood Volume: " + tier.getMaxVolume())
					.withStyle(ChatFormatting.GOLD));

			if (stack.hasTag()) {
				tooltip.add(Component.translatable("Blood Volume: " + bloodVolume.getBloodVolume())
						.withStyle(ChatFormatting.RED));
				if (stack.getTag().getBoolean(TAG_STATE)) {
					tooltip.add(Component.translatable("State: Open").withStyle(ChatFormatting.RED));
				} else {
					tooltip.add(Component.translatable("State: Corked").withStyle(ChatFormatting.GRAY));
				}
			}
		}
	}
