	...
	public void releaseUsing(ItemStack stack, Level worldIn, LivingEntity entityLiving, int timeLeft) {
		...
		if (i >= 10) {
			if (entityLiving instanceof Player) {
				 ...
				 * player.sendStatusMessage( Component.translatable("Not enough blood to be shed"),
				 * true); }
				 */
			}
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		...
		if (stack.hasTag()) {
			if (stack.getTag().getBoolean(TAG_STATE)) {
				tooltip.add(Component.translatable("State: Unleashed").withStyle(ChatFormatting.RED));
			} else {
				tooltip.add(Component.translatable("State: Tame").withStyle(ChatFormatting.GRAY));
			}
		}
	}
	@Override
...
class RenderPropLivingSpear implements IItemRenderProperties {
	...
	public BlockEntityWithoutLevelRenderer getItemStackRenderer() {
		return new RenderItemLivingSpear(Minecraft.getInstance().getBlockEntityRenderDispatcher(),
