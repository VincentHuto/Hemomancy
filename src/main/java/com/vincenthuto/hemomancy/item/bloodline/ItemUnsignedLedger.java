...
public class ItemUnsignedLedger extends Item {

	public static String TAG_STATE = "state";
	public static String TAG_BLOODLINE = "bloodline";

	public ItemUnsignedLedger(Properties prop) {
	...
	public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
		...
		if (stack.getItem() instanceof ItemUnsignedLedger) {
			...
			if (!compound.getBoolean(TAG_STATE)) {
				...
				if (!worldIn.isClientSide) {
					...
					if (volume.getBloodLine().getName().equals(savedLine.getName())) {
						playerIn.displayClientMessage( Component.translatable("Already in this bloodline"), true);
					} else {
	...
	public void appendHoverText(ItemStack stack, Level worldIn, List<Component> tooltip, TooltipFlag flagIn) {
		...
		if (stack.hasTag()) {
			if (!stack.getTag().getBoolean(TAG_STATE)) {
				tooltip.add(Component.translatable("Unsigned").withStyle(ChatFormatting.GRAY));
			} else {
				if (stack.getTag().contains(TAG_BLOODLINE)) {
					tooltip.add(Component.translatable("Signed with: "
