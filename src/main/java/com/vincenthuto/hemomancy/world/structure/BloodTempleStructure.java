...
public class BloodTempleStructure extends Structure {
	 ...
	 * extraSpawningChecks to return true only if certain chunk coordinates are
	 * passed in which allows you to spawn structures only at certain coordinates in
	 * the world.
	 *
	 ...
	 * structure on top of ledges. Best way to do that is to use getBaseColumn to
	 * grab a column of blocks at the structure's x/z position. Then loop through it
	 * and look for land with air above it and set blockpos's Y value to it. Make
	 * sure to set the final boolean in JigsawPlacement.addPieces to false so that
	 * the structure spawns at blockpos's y value instead of placing the structure
	 * on the Bedrock roof!
	 *
	 ...
	 * command will make minecraft hang forever and break the game. Use the biome
	 * tags for where to spawn the structure and users can datapack it to spawn in
	 * specific biomes that aren't in the dimension they don't like if they wish.
	 */
	private static boolean extraSpawningChecks(Structure.GenerationContext context) {
		// Grabs the chunk position we are at
		ChunkPos chunkpos = context.chunkPos();

		// Checks to make sure our structure does not spawn above land that's higher
		// than y = 150
		// to demonstrate how this method is good for checking extra conditions for
		// spawning
	...
	public Optional<Structure.GenerationStub> findGenerationPoint(Structure.GenerationContext context) {
		...
		// Check if the spot is valid for our structure. This is just as another method
		// for cleanness.
		// Returning an empty optional tells the game to skip this spot as it will not
		...
		Optional<Structure.GenerationStub> structurePiecesGenerator = JigsawPlacement.addPieces(context, // Used for
																											// JigsawPlacement
																											// to get
																											// all the
																											// proper
		 ...
		 * extremely powerful if you are doing something that vanilla's jigsaw system
		 * cannot do. Such as for example, forcing 3 pieces to always spawn every time,
		 * limiting how often a piece spawns, or remove the intersection limitation of
		 * pieces.
		 */
