 ...
 * within the Hematic Order. Degrees range from 0 (uninitiated) to 7 (Archon).
 */
public interface IInitiatoryDegree {

	/**
	 * Returns the player's current degree number (0 = uninitiated, 1–7 = a
	 * named degree).
	 */
	int getDegreeNumber();
	...
	/** Returns {@code true} if the player has reached the maximum degree (7). */
	boolean isMaxDegree();

	/**
	 * Sets the degree directly. Use {@code 0} to mark the player as uninitiated,
	 * or 1–7 to set a specific degree.
	 */
