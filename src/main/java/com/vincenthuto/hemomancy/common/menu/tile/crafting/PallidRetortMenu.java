 ...
 */
public class PallidRetortMenu extends AbstractContainerMenu {
	...
	private final PallidRetortBlockEntity container;
	private final ContainerData data;
	protected final Level level;
	private final PallidRetortBlockEntity te;

	// ---- Constructors ----

	private static PallidRetortBlockEntity getBlockEntity(final Inventory playerInv, final FriendlyByteBuf buf) {
		...
		if (tileAtPos instanceof PallidRetortBlockEntity be) {
			return be;
		}
	...
	public PallidRetortMenu(final int windowId, final Inventory playerInventory, final FriendlyByteBuf data) {
		this(windowId, playerInventory, getBlockEntity(playerInventory, data));
	}

	/** Server-side convenience (no explicit data container) */
	public PallidRetortMenu(int windowId, Inventory playerInventory, PallidRetortBlockEntity blockEntity) {
		this(windowId, playerInventory, blockEntity, new SimpleContainerData(DATA_COUNT));
	}

	/** Full constructor */
	public PallidRetortMenu(final int windowId, final Inventory playerInventory, final PallidRetortBlockEntity container,
			final ContainerData containerData) {
		super(ContainerInit.pallid_retort.get(), windowId);
		this.te = container;
	...
	public PallidRetortBlockEntity getTe() {
		return te;
	}
	...
	public boolean recipeMatches(DistillationRecipe recipe) {
		return recipe.matchesItems(
				this.container.getItem(PallidRetortBlockEntity.SLOT_INPUT),
				this.container.getItem(PallidRetortBlockEntity.SLOT_CATALYST));
	}
