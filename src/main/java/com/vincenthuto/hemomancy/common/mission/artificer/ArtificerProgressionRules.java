...
public final class ArtificerProgressionRules {
	...
	public static Set<String> forkResearchSpecimens(ForkFamily family) {
		return switch (family) {
			case BARBED -> Set.of("hemomancy:barbed_urchin", "hemomancy:desiccant",
					"hemomancy:venom_rib_centipede");
			case CHITINITE -> Set.of("hemomancy:chalybeate_snail", "hemomancy:chitinite",
					"hemomancy:fervent_chitinite");
			case PRISMATIC -> Set.of("hemomancy:prism_cuttle", "hemomancy:scarlet_serpent",
					"hemomancy:verdigris_moth");
			case NONE -> Set.of();
		};
	}

	public static int recordedForkResearchCount(ForkFamily family, Set<String> recordedSpecimens) {
		int count = 0;
		for (String specimen : forkResearchSpecimens(family)) {
			if (recordedSpecimens.contains(specimen)) count++;
		}
		return count;
	}

	public static boolean hasCompletedForkResearch(ForkFamily family, Set<String> recordedSpecimens) {
		Set<String> required = forkResearchSpecimens(family);
		return !required.isEmpty() && recordedSpecimens.containsAll(required);
	}

	public static boolean canClaimForkResearchReward(ForkFamily family, Set<String> recordedSpecimens,
			boolean correspondenceComplete, boolean claimed) {
		return correspondenceComplete && !claimed && hasCompletedForkResearch(family, recordedSpecimens);
	}
