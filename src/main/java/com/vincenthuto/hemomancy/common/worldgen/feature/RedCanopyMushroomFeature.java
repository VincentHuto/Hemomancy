...
public class RedCanopyMushroomFeature extends CanopyMushroomFeature {
	...
	protected void makeCap(LevelAccessor levelAccessor, RandomSource random, BlockPos pos, int height,
			...
			HugeMushroomFeatureConfiguration featureConfiguration) {
		if (this.altHeads <= 33) {
	...
	protected void makeVanillaCap(LevelAccessor levelAccessor, RandomSource random, BlockPos pos, int height,
			...
			for (int x = -j; x <= j; ++x) {
				for (int z = -j; z <= j; ++z) {
					...
					if (y >= height || xMinMax != zMinMax) {
						...
						if (!levelAccessor.getBlockState(mutableBlockPos).isSolidRender(levelAccessor,
								mutableBlockPos)) {
	...
	protected void makeSmoothCap(LevelAccessor levelAccessor, RandomSource random, BlockPos pos, int height,
			BlockPos.MutableBlockPos mutableBlockPos, HugeMushroomFeatureConfiguration featureConfiguration) {
						...
						if (!levelAccessor.getBlockState(mutableBlockPos).isSolidRender(levelAccessor,
							...
							if (blockstate.hasProperty(HugeMushroomBlock.WEST)
								...
								blockstate = blockstate
										.setValue(HugeMushroomBlock.UP,
												!isInsideSmoothShape(height, j - (y > height - 2 ? 1 : 0), x, y + 1, z))
										.setValue(HugeMushroomBlock.WEST,
												!isInsideSmoothShape(height, j, x - 1, y, z) && x < 0)
										.setValue(HugeMushroomBlock.EAST,
												!isInsideSmoothShape(height, j, x + 1, y, z) && x > 0)
										.setValue(HugeMushroomBlock.NORTH,
												!isInsideSmoothShape(height, j, x, y, z - 1) && z < 0)
										.setValue(HugeMushroomBlock.SOUTH,
												!isInsideSmoothShape(height, j, x, y, z + 1) && z > 0);
							}
	...
	/**
	 * Don't know if there's a cleaner way of doing this? Feel free to try, but this
	 * works for now. Most of this code is just for the blockStates to be correct,
	 * this would be a lot shorter otherwise and wouldn't be a method.
	 */
	...
	protected void makeSpheroidCap(LevelAccessor levelAccessor, RandomSource random, BlockPos pos, int height,
			BlockPos.MutableBlockPos mutableBlockPos, HugeMushroomFeatureConfiguration featureConfiguration) {
					...
					if (distance <= maxDistance) {
						...
						if (!levelAccessor.getBlockState(mutableBlockPos).isSolidRender(levelAccessor,
							...
							if (blockstate.hasProperty(HugeMushroomBlock.WEST)
								...
								blockstate = blockstate
										.setValue(HugeMushroomBlock.UP,
												y > height - 2 && (y == height || distance > maxDistance - 1D))
										.setValue(HugeMushroomBlock.WEST,
												Math.sqrt((x - 1) * (x - 1) + z * z) > maxDistance)
										.setValue(HugeMushroomBlock.EAST,
												Math.sqrt((x + 1) * (x + 1) + z * z) > maxDistance)
										.setValue(HugeMushroomBlock.NORTH,
												Math.sqrt(x * x + (z - 1) * (z - 1)) > maxDistance)
										.setValue(HugeMushroomBlock.SOUTH,
												Math.sqrt(x * x + (z + 1) * (z + 1)) > maxDistance);
							}
