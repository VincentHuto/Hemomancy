...
public class ItemFungalRune extends ItemRune implements IRenderRune {

	public ItemFungalRune(Properties properties, EnumBloodTendency tendencyIn, float deepenAmountIn) {
	...
	public RuneType getRuneType() {
		return RuneType.FUNGAL;
	}

	@Override
	public void onPlayerRuneRender(PoseStack matrix, ItemStack stack, int packedLight, MultiBufferSource iRenderTypeBuffer,
			Player player, float partialTicks) {
		Minecraft mc = Minecraft.getInstance();

		matrix.pushPose();
		matrix.mulPose(Vector3.ZP.rotationDegrees(180).toMoj());
		matrix.mulPose(Vector3.YP.rotationDegrees(player.level().getGameTime()).toMoj());
