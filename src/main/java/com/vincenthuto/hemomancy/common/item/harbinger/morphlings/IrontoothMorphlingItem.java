...
/**
 * Irontooth strain that grants increased mining speed and bonus underground
 ...
 * speed bonus. Prefers FERRIC (iron-rich soil nourishes the teeth) with
 ...
 *   ores and entities glow briefly (tremor-sense through vibration)
 * - Mature (3): Earthen Bulwark — when taking damage underground, gain brief
 *   Resistance (the strain retreats into compacted earth)
 * - Apex (4): Seismic Slam — while sneaking and jumping underground, emit a
 *   shockwave that damages and launches nearby hostiles
 */
public class IrontoothMorphlingItem extends MorphlingItem {
	...
	private static final int SHED_COOLDOWN = 1200;
	private static final int HARVEST_PROGRESS_INTERVAL = 12;
	/** Y-level below which the strain is considered underground. */
	private static final double UNDERGROUND_THRESHOLD = 50.0;

	public IrontoothMorphlingItem(Properties prop) {
		super(prop);
	}

	@Override
	protected String binomialKey() {
		return "morphling.hemomancy.irontooth.binomial";
	}
	...
	public void onEquippedBlockBreak(Player player, ItemStack stack, BlockPos pos, BlockState state) {
		if (player.level().isClientSide || pos.getY() >= 0) {
			return;
		}
		int progress = MorphlingItem.addHusbandryProgress(stack, 1);
		if (progress > 0 && progress % HARVEST_PROGRESS_INTERVAL == 0) {
			tryShedUnderground(player, stack, 1);
		}
	}

	private void tryShedUnderground(Player player, ItemStack stack, int count) {
		if (player.level().isClientSide || player.getY() >= UNDERGROUND_THRESHOLD || count <= 0) {
			return;
		}
		long now = player.level().getGameTime();
		long lastShed = getLastAbilityTick(stack, "UndergroundShed");
		if (now - lastShed < SHED_COOLDOWN) {
			return;
		}
		setLastAbilityTick(stack, "UndergroundShed", now);
		if (player.level() instanceof ServerLevel serverLevel) {
			ItemEntity drop = new ItemEntity(serverLevel, player.getX(), player.getY(), player.getZ(),
					new ItemStack(ItemInit.chalybeate_sclerite.get(), count));
