...
public class FerventChitiniteEntity extends PathfinderMob {

	private static final EntityDataAccessor<Boolean> ROLLED_UP =
			SynchedEntityData.defineId(FerventChitiniteEntity.class, EntityDataSerializers.BOOLEAN);
	...
	public FerventChitiniteEntity(EntityType<? extends FerventChitiniteEntity> type, Level worldIn) {
		super(type, worldIn);
	}
	...
	protected SoundEvent getAmbientSound() {
		return SoundInit.ENTITY_FERVENT_CHITINITE_AMBIENT.get();
	}

	@Override
	protected SoundEvent getDeathSound() {
		return SoundInit.ENTITY_FERVENT_CHITINITE_DEATH.get();
	}

	@Override
	protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
		return SoundInit.ENTITY_FERVENT_CHITINITE_HURT.get();
	}
	...
	public void sporePuff(Level world, AABB effectBounds, double x, double y, double z) {
		...
		for (Entity ent : list) {
			if (!(ent instanceof FerventChitiniteEntity)) {
	...
	public void tick() {
		...
		if (isClose) {
			yHeadRot = (float) MathUtils.getAngle(FerventChitiniteEntity.this, target) + 90f;
		}
	}
	...
	private class RollupGoal extends Goal {
		private final FerventChitiniteEntity chitinite;
		private int rollupTimer;
		private int cooldown;

		public RollupGoal(FerventChitiniteEntity chitinite) {
