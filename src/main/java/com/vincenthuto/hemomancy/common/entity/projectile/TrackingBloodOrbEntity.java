...
public class TrackingBloodOrbEntity extends ThrowableProjectile {
	public static EntityType<TrackingBloodOrbEntity> TYPE = EntityInit.tracking_blood_orb.get();

	private static final String TAG_TIME = "time";
	private static final EntityDataAccessor<Boolean> EVIL = SynchedEntityData.defineId(TrackingBloodOrbEntity.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> TARGET = SynchedEntityData.defineId(TrackingBloodOrbEntity.class,
			EntityDataSerializers.INT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public TrackingBloodOrbEntity(EntityType<TrackingBloodOrbEntity> type, Level world) {
		super(type, world);
	}

	public TrackingBloodOrbEntity(Level world) {
		this(TYPE, world);
	}

	public TrackingBloodOrbEntity(LivingEntity thrower, boolean evil) {
	...
	public void tick() {
		...
		if (level().isClientSide) {
			level().addParticle(HemoParticleData.glow(new ParticleColor(200, 0, 0)),
