...
public class MenuJuiceinator extends AbstractContainerMenu {
	...
	private final BlockEntityJuicinator container;
	private final ContainerData data;
	protected final Level level;
	private final RecipeType<? extends AbstractCookingRecipe> recipeType;
	private BlockEntityJuicinator te;

	public MenuJuiceinator(final int windowId, final Inventory playerInventory, final FriendlyByteBuf data) {
		this(windowId, playerInventory, getBlockEntity(playerInventory, data));
	}

	public MenuJuiceinator(int windowIdIn, Inventory playerInventoryIn, BlockEntityJuicinator blockEntityJuicinator) {
		this(windowIdIn, playerInventoryIn, blockEntityJuicinator, new SimpleContainerData(4));
	}

	public MenuJuiceinator(final int windowId, final Inventory playerInventory, final BlockEntityJuicinator container,
			final ContainerData containerData) {
		super(ContainerInit.juiceinator.get(), windowId);
		this.te = container;
		this.recipeType = RecipeInit.juiceinator.get();
	...
	public BlockEntityJuicinator getTe() {
		return te;
	}
	...
	public boolean recipeMatches(Recipe<? super BlockEntityJuicinator> p_38980_) {
	...
	private static BlockEntityJuicinator getBlockEntity(final Inventory playerInv, final FriendlyByteBuf data) {
		Objects.requireNonNull(playerInv, "playerInventory cannot be null");
		Objects.requireNonNull(data, "data cannot be null");
		final BlockEntity tileAtPos = playerInv.player.level.getBlockEntity(data.readBlockPos());
		if (tileAtPos instanceof BlockEntityJuicinator) {
			return (BlockEntityJuicinator) tileAtPos;
		}
