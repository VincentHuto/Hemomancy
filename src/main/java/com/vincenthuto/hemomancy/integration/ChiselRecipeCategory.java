...
public class ChiselRecipeCategory implements IRecipeCategory<ChiselRecipe> {
	...
	public RecipeType<ChiselRecipe> getRecipeType() {
		return JEIPlugin.runic_chisel_station_recipe_type;
	}
	...
	@Override
	public void draw(ChiselRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack matrixStack, double mouseX,
			double mouseY) {
	...
	public void setRecipe(IRecipeLayoutBuilder builder, ChiselRecipe recipe, IFocusGroup focuses) {
		builder.addSlot(RecipeIngredientRole.INPUT, 4, 11).addIngredients(VanillaTypes.ITEM_STACK,
				Arrays.asList(Ingredient.of(HLItemInit.TAG_KNAPPERS).getItems()));
		builder.addSlot(RecipeIngredientRole.RENDER_ONLY, 22, 0).addIngredient(VanillaTypes.ITEM_STACK,
				new ItemStack(BlockInit.runic_chisel_station.get()));
		...
		if (!recipe.getIngredient1().isEmpty() && !recipe.getIngredient2().isEmpty()) {
			builder.addSlot(RecipeIngredientRole.INPUT, 4, 33).addIngredients(VanillaTypes.ITEM_STACK,
					Arrays.asList(recipe.getIngredient1().getItems()));
			builder.addSlot(RecipeIngredientRole.INPUT, 4, 55).addIngredients(VanillaTypes.ITEM_STACK,
			...
			builder.addSlot(RecipeIngredientRole.INPUT, 4, 33).addIngredients(VanillaTypes.ITEM_STACK,
					Arrays.asList(recipe.getIngredient1().getItems()));
			builder.addSlot(RecipeIngredientRole.OUTPUT, 115, 44).addIngredient(VanillaTypes.ITEM_STACK,
					recipe.getResultItem());
		}
