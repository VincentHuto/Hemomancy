...
public class VascularSystemPacketServer {
	private Map<EnumVeinSections, Float> vascularSystem = new HashMap<>();

	public VascularSystemPacketServer(Map<EnumVeinSections, Float> vascularSystemIn) {
		this.vascularSystem = vascularSystemIn;
	}

	// This code only runs on the client
	@SuppressWarnings("unused")
	public static void handle(final VascularSystemPacketServer msg, Supplier<NetworkEvent.Context> ctx) {
		ctx.get().enqueueWork(() -> {
	...
	public static void encode(final VascularSystemPacketServer msg, final PacketBuffer packetBuffer) {
	...
	public static VascularSystemPacketServer decode(final PacketBuffer packetBuffer) {
		Map<EnumVeinSections, Float> devo = new HashMap<>();
		for (EnumVeinSections key : EnumVeinSections.values()) {
			devo.put(key, packetBuffer.readCompoundTag().getFloat(key.toString()));
		}
		return new VascularSystemPacketServer(devo);
	}
}