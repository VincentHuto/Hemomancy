									...
									if (hitBlock == targetPattern.getHitBlock()) {
										...
										if (patternHelper != null) {
											for (int i = 0; i < targetPattern.getBundledPattern().getBlockPattern()
												...
												for (int j = 0; j < targetPattern.getBundledPattern().getBlockPattern()
													...
													for (int k = 0; k < targetPattern.getBundledPattern()
															.getBlockPattern().getFingerLength(); ++k) {
														CachedBlockInfo cachedBlockInfo = patternHelper
																.translateOffset(i, j, k);
														sWorld.setBlockState(cachedBlockInfo.getPos(),
											...
											sWorld.playSound(hitPos.getX(), hitPos.getY(), hitPos.getZ(),
													SoundEvents.ENTITY_ENDERMAN_SCREAM, SoundCategory.BLOCKS, 1, 1,
													true);
											...
											PacketHandler.CHANNELBLOODVOLUME.send(
													...
													new PacketBloodVolumeServer(bloodVolume.getBloodVolume()));
										}
									}
								}
							} else {
								player.sendStatusMessage(
										new StringTextComponent("Not enough blood can be drawn for formation"), true);
								sWorld.playSound(player.getPosition().getX(), player.getPosition().getY(),
										player.getPosition().getZ(), SoundEvents.ENTITY_ENDERMAN_SCREAM, null, 1f, 1f,
										false);
							}
						}
