 ...
 * net.minecraft.item.ItemStack; import net.minecraft.network.PacketBuffer;
 * import net.minecraft.util.CachedBlockInfo; import net.minecraft.util.Hand;
 ...
 * net.minecraft.world.server.ServerWorld; import
 ...
 * stack; }
 * 
 * public static PacketBloodCraftingKeyPressOld decode(final PacketBuffer
 ...
 * PacketBuffer buffer) { buffer.writeByte(0);
 ...
 * PlayerEntity player = ctx.get().getSender(); if (player == null) return;
 ...
 * ServerWorld sWorld = (ServerWorld) ctx.get().getSender().world; if
 ...
 * hitBlock = sWorld.getBlockState(hitPos).getBlock(); if (hitBlock ==
 * targetPattern.getHitBlock()) { BlockPattern.PatternHelper patternHelper =
 * targetPattern.getBundledPattern() .getBlockPattern().match(sWorld, hitPos);
 ...
 * 2); sWorld.playEvent(2001, cachedBlockInfo.getPos(),
 ...
 * sWorld.addEntity(new ItemEntity(sWorld, hitPos.getX(), hitPos.getY(),
 * hitPos.getZ(), new ItemStack(targetPattern.getCreation()))); ItemStack
 * oldStack = player.getHeldItemMainhand().copy();
 * player.setHeldItem(Hand.MAIN_HAND, new ItemStack(oldStack.getItem(),
 * oldStack.getCount() - 1));
 ...
 * StringTextComponent("Not enough blood can be drawn for formation"), true);
