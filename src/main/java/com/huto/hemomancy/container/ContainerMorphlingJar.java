...
public class ContainerMorphlingJar extends AbstractContainerMenu {
	public ContainerMorphlingJar(final int windowId, final Inventory playerInventory) {
		this(windowId, playerInventory.player.level, playerInventory.player.blockPosition(), playerInventory,
				playerInventory.player);
	}

	public ContainerMorphlingJar(int windowId, Level world, BlockPos pos, Inventory playerInventory,
			Player playerEntity) {
		super(type, windowId);
	...
	public ContainerMorphlingJar(int openType, int windowId, Level world, BlockPos pos, Inventory playerInventory,
			Player playerEntity) {
	...
	public static final MenuType type = new MenuType<>(ContainerMorphlingJar::new)
	...
	public boolean stillValid(Player playerIn) {
		...
		return playerIn.inventory.getItem(slotID).getItem() instanceof ItemMorphlingJar;
	}

	@Override
	public ItemStack clicked(int slot, int dragType, ClickType clickTypeIn, Player player) {
		if (slot >= 0) {
			if (getSlot(slot).getItem().getItem() instanceof ItemMorphlingJar)
