...
public class RenderCape {
	static final int PLAYER_SKIP_RANGE = 4 * 4;
	static final int FLUID_CACHE_CLEAR_RATE = 4;
	static final int FLUID_CACHE_CLEAR_SIZE = 6;
	static final int ITERATIONS = 11;
	static final float DELTA_TIME = 0.0052F;
	static final float GRAVITY = -2800;
	static final float FLUID_FORCE = -100;
	static final int HEIGHT = 8;
	static final int WIDTH = 4;
	final ImmutableList<Point> points;
	final ImmutableList<Quad> quads;
	final Long2BooleanMap fluidCache = new Long2BooleanOpenHashMap(10);
	final BlockPos.Mutable scratchPos = new BlockPos.Mutable();
	double posX;
	double posY;
	double posZ;

	RenderCape(ImmutableList<Point> points, ImmutableList<Quad> quads) {
		this.points = points;
		this.quads = quads;
	}

	public static RenderCape create() {
	...
	private void updatePoints(PlayerEntity player) {
	 ...
	 void updateFluidCache(PlayerEntity player) {
		if (player.ticksExisted % FLUID_CACHE_CLEAR_RATE == 0 && fluidCache.size() > FLUID_CACHE_CLEAR_SIZE) {
			fluidCache.clear();
		}
	}

	 boolean isFluid(World world, float x, float y, float z) {
	...
	void render(PlayerEntity player, double x, double y, double z, float delta) {
		...
		for (Quad quad : quads) {
			...
			ny /= len;
			nz /= len;
			buf.pos(v00x, v00y, v00z).tex((float) v00.getU(), (float) v00.getV()).endVertex();
			buf.pos(v01x, v01y, v01z).tex((float) v01.getU(), (float) v01.getV()).endVertex();
			buf.pos(v11x, v11y, v11z).tex((float) v11.getU(), (float) v11.getV()).endVertex();
			buf.pos(v10x, v10y, v10z).tex((float) v10.getU(), (float) v10.getV()).endVertex();
		}

		ResourceLocation fallBackCape;
		fallBackCape = new ResourceLocation(Hemomancy.MOD_ID, "textures/item/crimson_item_glint.png");
	...
	private static final class PlayerCollisionResolver extends PlayerResolver {
		...
		public void resolve(PlayerEntity player, Point point) {
			...
			if (player.isSneaking()) {
				...
				dy /= len;
				dz /= len;
				float rz = (point.posX - (float) player.getPosX()) * dx + (point.posZ - (float) player.getPosX()) * dz;
