...
public final class CapeEvent {
	...
	public static void renderLevelLast(RenderWorldLastEvent event) {
		...
		if (!player.isInvisible() && !player.isFallFlying() && !player.isSleeping() && delta != 1) {
			...
			if (cape.isPresent(player)) {
				...
				cape.render(event.getMatrixStack(), player, locVec.x, locVec.y, locVec.z, delta);
			}
		}
	...
	public static void onClientTick(TickEvent.ClientTickEvent event) {
		if (event.side == LogicalSide.CLIENT && event.phase == TickEvent.Phase.END) {
			Level world = ClientUtils.getLevel();
			if (world != null) {
	...
	private static final class RenderCape {
		...
		void render(MatrixStack matrixStack, Player player, double x, double y, double z, float delta) {
			...
			for (Quad quad : quads) {
				...
				float a = (0xFF00FF >> 24 & 255) / 255.0F;
				float r = (0xFF00FF >> 16 & 255) / 255.0F;
				float g = (0xFF00FF >> 8 & 255) / 255.0F;
				float b = (0xFF00FF & 255) / 255.0F;
				...
				bufferbuilder.vertex(matrix, v01x, v01y, v01z).color(r, g, b, a)
						.uv((float) v01.getU(), (float) v01.getV()).endVertex();
