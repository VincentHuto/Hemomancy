...
public class RenderDSD extends ItemStackTileEntityRenderer {

	private IBakedModel defaultSpellModel;
	private final ResourceLocation location = new ResourceLocation(Hemomancy.MOD_ID, "textures/item/drudge_submission_device_texture");
	...
	private void spawnFirstPersonParticlesForStack(ItemStack stack, HandSide hand) {
		...
		if (!(stack.getItem() instanceof ItemDSD)) {
			return;
		}
		Minecraft mc = Minecraft.getInstance();
		...
		if (!itemIsInUse || playerIsRightHanded && activeHand == Hand.MAIN_HAND && hand == HandSide.LEFT
			...
			Vector3d offset = new Vector3d(player.world.rand.nextGaussian(), player.world.rand.nextGaussian(),
					player.world.rand.nextGaussian()).normalize().scale((double) 0.3f);
			origin = origin.add(offset);
			player.world.addParticle(GlowParticleData.createData(new ParticleColor(255, 100, 50)), origin.x, origin.y,
					origin.z, 0, 0, 0);
			player.world.addParticle(GlowParticleData.createData(new ParticleColor(255, 255, 50)), origin.x, origin.y,
					origin.z, 0, 0, 0);
			player.world.addParticle(GlowParticleData.createData(new ParticleColor(100, 100, 50)), origin.x, origin.y,
					origin.z, 0, 0, 0);
		}
	}
}