...
public class ParticleLineData implements IParticleFactory<ColoredDynamicTypeData> {
	...
	@Override
	public Particle makeParticle(ColoredDynamicTypeData data, ClientWorld worldIn, double x, double y, double z,
	...
	public static IParticleData createData(ParticleColor color) {
		return new ColoredDynamicTypeData(ParticleInit.line.get(), color, (float) ParticleUtil.inRange(0.05, 0.15),
				40 + random.nextInt(20));
	}

	public static IParticleData createData(ParticleColor color, float scale, int age) {
		return new ColoredDynamicTypeData(ParticleInit.line.get(), color, scale, age);
	}
}