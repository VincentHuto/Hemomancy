...
public class EntityChthonianQueen extends PathfinderMob implements IAnimatable {
	...
	public void sporePuff(Level world, AABB effectBounds, double x, double y, double z) {
		...
		for (Entity ent : list) {
			if (!(ent instanceof EntityChthonianQueen)) {
	...
	public void tick() {
		...
		if (isClose) {
			yRot = (float) MathUtils.getAngle(EntityChthonianQueen.this, target) + 90f;
		}
		if (noActiveAnimation()) {
			if (isClose && Mth.degreesDifferenceAbs(
					(float) MathUtils.getAngle(EntityChthonianQueen.this, target) + 90, yRot) < 30) {
