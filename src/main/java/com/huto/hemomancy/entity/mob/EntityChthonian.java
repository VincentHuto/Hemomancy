...
public class EntityChthonian extends CreatureEntity implements IAnimatable {
	...
	public EntityChthonian(EntityType<? extends EntityChthonian> type, World worldIn) {
		super(type, worldIn);
	...
	public void sporePuff(World world, AxisAlignedBB effectBounds, double x, double y, double z) {
		...
		for (Entity ent : list) {
			if (!(ent instanceof EntityChthonian)) {
	...
	public void tick() {
		...
		if (isClose) {
			rotationYaw = (float) MathUtil.getAngle(EntityChthonian.this, target) + 90f;
		}
		if (noActiveAnimation()) {
			if (isClose && MathHelper.degreesDifferenceAbs((float) MathUtil.getAngle(EntityChthonian.this, target) + 90,
					rotationYaw) < 30) {
