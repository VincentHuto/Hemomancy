...
public class EntityChthonian extends CreatureEntity implements IAnimatable {
	...
	public void tick() {
		...
		if (isClose) {
			rotationYaw = (float) Mafs.getAngle(EntityChthonian.this, target) + 90f;
		}
		if (noActiveAnimation()) {
			if (isClose && MathHelper.degreesDifferenceAbs((float) Mafs.getAngle(EntityChthonian.this, target) + 90,
					rotationYaw) < 30) {
