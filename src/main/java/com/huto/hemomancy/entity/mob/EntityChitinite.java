...
public class EntityChitinite extends CreatureEntity implements IAnimatable {
	...
	public void tick() {
		...
		if (isClose) {
			rotationYaw = (float) MathUtils.getAngle(EntityChitinite.this, target) + 90f;
		}
		if (noActiveAnimation()) {
			if (isClose
					&& MathHelper.degreesDifferenceAbs((float) MathUtils.getAngle(EntityChitinite.this, target) + 90,
							rotationYaw) < 30) {
