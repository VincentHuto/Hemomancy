...
public class EntityTrackingPests extends ThrowableProjectile {
	...
	private static final EntityDataAccessor<Boolean> EVIL = SynchedEntityData.defineId(EntityTrackingPests.class,
			EntityDataSerializers.BOOLEAN);
	private static final EntityDataAccessor<Integer> TARGET = SynchedEntityData.defineId(EntityTrackingPests.class,
			EntityDataSerializers.INT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public EntityTrackingPests(EntityType<EntityTrackingPests> type, Level world) {
		super(type, world);
	}

	public EntityTrackingPests(Level world) {
		this(TYPE, world);
	}
