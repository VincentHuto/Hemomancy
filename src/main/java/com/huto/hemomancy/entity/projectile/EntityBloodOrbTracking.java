...
public class EntityBloodOrbTracking extends ThrowableEntity {
	public static EntityType<EntityBloodOrbTracking> TYPE = EntityInit.tracking_blood_orb.get();

	private static final String TAG_TIME = "time";
	private static final DataParameter<Boolean> EVIL = EntityDataManager.createKey(EntityBloodOrbTracking.class,
			DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> TARGET = EntityDataManager.createKey(EntityBloodOrbTracking.class,
			DataSerializers.VARINT);

	double lockX, lockY = -1, lockZ;
	int time = 0;

	public EntityBloodOrbTracking(EntityType<EntityBloodOrbTracking> type, World world) {
		super(type, world);
	}

	public EntityBloodOrbTracking(World world) {
		this(TYPE, world);
	}
