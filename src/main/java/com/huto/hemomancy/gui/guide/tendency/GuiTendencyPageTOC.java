	...
	protected void init() {
		...
		this.addButton(buttonTitle = new GuiButtonTextured(texture, TITLEBUTTON, left - guiWidth + 150,
				top + guiHeight - 209, 24, 16, 174, 32, null, (press) -> {
					if (ClientUtils.getClientPlayer().getMainHandItem().getItem() == ItemInit.liber_inclinatio_hidden
							.get()) {
						mc.setScreen(new GuiTendencyTitlePage(true));
					} else {
						mc.setScreen(new GuiTendencyTitlePage(false));
					}
				}));
		...
		this.addButton(
				arrowB = new GuiButtonBookArrow(ARROWB, left, top + guiHeight - 10, 16, 14, 192, 1, new IPressable() {
					@Override
					public void onPress(Button p_onPress_1_) {
						if (ClientUtils.getClientPlayer().getMainHandItem()
