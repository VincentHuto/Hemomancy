...
public class GuiChiselStation extends AbstractContainerScreen<ContainerChiselStation> {
	...
	public GuiChiselStation(ContainerChiselStation screenContainer, PlayerInventory inv, Component titleIn) {
	...
	protected void renderLabels(MatrixStack matrixStack, int x, int y) {
		...
		if (te.hasValidRecipe()) {
			...
			renderTooltip(matrixStack, new TextComponent(I18n.get(currentRecipe.getOutput().getDescriptionId())),
					250, 150);
	...
	protected void renderBg(MatrixStack matrixStack, float partialTicks, int x, int y) {
		...
		GuiUtils.drawTexturedModalRect(this.leftPos, this.topPos, 0, 0, this.imageWidth, this.imageHeight, 0f);
	...
	protected void init() {
		...
		this.addButton(clearButton = new GuiButtonTextured(GUI_Chisel, CLEARBUTTONID,
				left + guiWidth - (guiWidth - 120), top + guiHeight - (170), 16, 16, 176, 16, null, (press) -> {
					...
					}}));this.addButton(chiselButton=new GuiButtonTextured(GUI_Chisel,CHISELBUTTONID,left+guiWidth-(guiWidth-120),top+guiHeight-(150),16,16,176,48,null,(press)->{if(te.chestContents.get(3).getItem()!=Items.AIR){PacketHandler.CHANNELMAIN.sendToServer(new PacketChiselCraftingEvent());for(
