...
public class RuneEntityEventHandler {
	...
	public static void attachCapabilitiesPlayer(AttachCapabilitiesEvent<Entity> event) {
		if (event.getObject() instanceof PlayerEntity) {
			event.addCapability(new ResourceLocation(Hemomancy.MOD_ID, "runecontainer"),
					new RunesContainerProvider((PlayerEntity) event.getObject()));
		}
	}
	...
	public static void syncSlot(PlayerEntity player, byte slot, ItemStack stack,
		...
		player.getCapability(RunesCapabilities.RUNES).ifPresent(runes -> {
			for (int i = 0; i < runes.getSlots(); ++i) {
				if (!runes.getStackInSlot(i).isEmpty()) {
					ItemEntity ei = new ItemEntity(player.world, player.getPosX(),
							player.getPosY() + player.getEyeHeight(), player.getPosZ(), runes.getStackInSlot(i).copy());
