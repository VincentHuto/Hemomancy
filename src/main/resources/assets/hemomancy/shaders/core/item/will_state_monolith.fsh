...
uniform vec3 WillMonolithColor;
...
void main() {
    ...
    vec3 tint = max(WillMonolithColor, vec3(0.0));
    vec3 black = tint * blackDepth * vec3(0.38, 0.05, 0.05);
    vec3 monolithColor = tint * (0.62 + Burden * 0.30);
    vec3 color = mix(black, monolithColor, clamp(redMemory, 0.0, 0.92));
    float veinGlow = clamp(flow * (1.45 + Burden * 0.35 + Attuned * 0.20), 0.0, 1.0);
    color += tint * (0.82 + Burden * 0.20) * veinGlow;
    color += tint * 0.18 * edge * (0.35 + Burden * 0.45);
    float alpha = clamp(0.92 + fault * 0.08 + veinGlow * 0.09 + edge * 0.05, 0.0, 1.0);
    alpha *= WillDissolveFlicker * (1.0 - WillDissolveProgress * 0.28);
