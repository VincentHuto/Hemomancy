...
public final class PuppeteerIntegrationSourceTest {
	...
	public static void main(String[] args) throws Exception {
		...
		String vultureTrial = read("src/main/resources/data/hemomancy/recipe/puppeteer_trial/veinwing_vulture.json");
		String spitterTrial = read("src/main/resources/data/hemomancy/recipe/puppeteer_trial/marrow_spitter.json");
		String hulkTrial = read("src/main/resources/data/hemomancy/recipe/puppeteer_trial/gorebound_hulk.json");
		String mnemonistTrial = read("src/main/resources/data/hemomancy/recipe/puppeteer_trial/mnemonist_puppet.json");
		...
		assertContains(craftingHelper, "PuppeteerTrialRecipe.getAllTrialRecipes(level)");
		...
		assertContains(summonsTab, "trialCatalyst(definition)");
		assertContains(craftingController, "PuppeteerTrialRecipe.getAllTrialRecipes(mc.level)");
		assertContains(craftingController, "if (!unstained && mc.player != null && mc.level != null)");
		assertContains(craftingView, "recipe instanceof PuppeteerTrialRecipe trial");
		assertContains(craftingView, "Trial Reward:");
		assertContains(craftingView, "Hold the catalyst in your offhand and project blood");
		assertContains(spawnerScreen, "PuppeteerTrialRecipe.getAllTrialRecipes(Minecraft.getInstance().level)");
		assertContains(spawnerScreen, "PlaceStructurePacket.StructureType.PUPPETEER_TRIAL");
		assertContains(spawnerScreen, "ChatFormatting.YELLOW");
		assertContains(spawnerScreen, "[Puppet Trial] ");
		assertContains(placeStructurePacket, "PUPPETEER_TRIAL");
		assertContains(placeStructurePacket, "PuppeteerTrialRecipe.getAllTrialRecipes(level)");
		assertContains(vultureTrial, "[\"VEVEV\"],\n    [\"EEEEE\"],\n    [\"VECEV\"]");
		assertContains(spitterTrial, "[\"VEBEV\"],\n    [\"VECEV\"],\n    [\"EEEEE\"]");
		assertContains(hulkTrial, "[\"EEEEE\"],\n    [\"EGGGE\"],\n    [\"EGCGE\"]");
		assertContains(mnemonistTrial, "[\"EMEME\"],\n    [\"MCCCM\"],\n    [\"ECQCE\"]");
