...
class IronHeartAttachmentTest {
    @Test void anchorStaysLeftAndForwardThroughFullTurn() {
        for(int yaw=0;yaw<360;yaw+=15) {
            ...
            assertEquals(.12,anchor.x*Math.cos(angle)+anchor.z*Math.sin(angle),1e-6);
            assertEquals(.15,-anchor.x*Math.sin(angle)+anchor.z*Math.cos(angle),1e-6);
            assertEquals(1.35,anchor.y,1e-6);
    ...
    @Test void crouchedAndScaledBodyKeepsItsAttachment() {
        ...
        Vector3f anchor=poses.last().pose().transformPosition(new Vector3f());
        assertEquals(.24,anchor.x,1e-6);
        assertEquals(2.24,anchor.y,1e-6);
