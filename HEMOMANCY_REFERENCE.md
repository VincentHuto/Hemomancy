...
Opened from the **Dendritic Distributor** block. Has six tabs: **Skills**, **Manipulations**, **Crafting**, **Runes**, **Rites**, and **Materials**. The Crafting tab shows blood structure recipes grouped by tier (Basic/Advanced/Expert). The Runes tab shows chisel recipes grouped by tier (1/2/3) with degree gating (4/4/5). The Rites tab shows cardinal rite recipes. The Materials tab displays a catalogue of mod items and blocks.
...
| ![](src/main/resources/assets/hemomancy/textures/item/memory_umbral_step.png) Umbral Step | ![](src/main/resources/assets/hemomancy/textures/item/memory_summon_thrall.png) Summon Thrall | memory_cryogenic_pulse Cryogenic Pulse | memory_glacial_bastion Glacial Bastion |
| memory_sanguine_ignition Sanguine Ignition | memory_vitric_combustion Vitric Combustion | memory_void_shroud Void Shroud | memory_blood_eclipse Blood Eclipse |
| memory_hemorrhage Hemorrhage | memory_exsanguinate Exsanguinate | | |
...
- **Skill Effect Wiring** — ~~7 of 13 skills are not wired.~~ **PARTIALLY RESOLVED:** All 18 skills in `SkillPointHelper` have helper methods. Fully wired into event handlers: Capacity, Efficiency, Last Wind, Feeding Frenzy, Crimson Mastery, Sanguine Reach. Still helper-only (not yet called from events): Dynamic Use, Hemostasis, Sanguine Surge, Vital Link, Iron Will, Blood Flow, Coagulation, Manip Slots, Rune Affinity, Rune Resonance, Rune Mastery.
