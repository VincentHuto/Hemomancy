...
| **Putriciel** | Absolution-window victory condition — players must deal damage during brief periodic absolution cycles (opens every 300 ticks, lasts 80 ticks); requires 5 successful absolution hits. Rot nova pulses Wither+fire to the whole arena. | `PutricielEntity`: `DATA_ABSOLVED` synched flag; `openAbsolutionWindow()` / `endAbsolutionWindow()` cycle; `hurt()` increments absolution counter during window; on 5th absolution entity is discarded and drops Hallowed Residuum | MORTEM + FLAMMEUS (absolution, rot-fire) |
| **Velorum** | Martyrdom resistance — gains brief Resistance I on every hit, creating attack-rhythm windows. Frost nova roots players. Veil of darkness blinds (Nausea for blood-active players). Silence drain strips blood from nearby Harbingers at low HP (≤25%). | `VelorumEntity`: `DATA_MARTYRDOM` synched flag; `fireFrostNova()`, `fireVeilOfDarkness()`, `fireSilenceDrain()` per-tick methods; martyrdom Resistance in `hurt()` | CONGEATIO + TENEBRIS (martyrdom, silence, frozen dark) |

> **⚠️ Partially Implemented:** Saints 2–4 (Seraphae, Putriciel, Velorum) have boss AI implemented (`SeraphaeEntity`, `PutricielEntity`, `VelorumEntity`) and are registered in `EntityInit`. Trial Chamber structure generation (the arena rooms in world) is still WIP. Models/textures/GeckoLib animations are stub/placeholder. Hemorath boss fight mechanic remains WIP.
...
| **Field Notes** | Stack-local memo notebook. Captures fleeting dialogue/memo events into `DataComponents.CUSTOM_DATA` (`Memos`, `RemainingMemos`). Holds 15 memo captures per Hematic Field Ink refill. Field Notes do not become their own Liber chapter; dictation unlocks normal Liber Sanguinum chapter pages in the player's `LiberKnowledge` attachment. |
| **Hematic Field Ink** | Field Notes refill item crafted from Dicentra Sap, Hematic Iron Powder, a water bottle, and an ink sac. |
...
> **RESOLVED:** Organ extraction ritual (Visceral Mirror → cycle organs → confirm → produce Echo items) and all per-organ gameplay effects are fully implemented in `VisceralOrgansEvents` (player tick + capability check): **Spleen** +1000 max blood per organ level; **Liver** removes Poison (level 2+) and Wither (level 3+) on tick; **Lungs** grants Water Breathing (100×level ticks) while underwater; **Kidneys** grants Regeneration (level-1 amplifier); **Heart** grants Damage Resistance but drains 10×level blood per 2s tick.
...
| **Annetta Knowles (The Stained Priestess)** | | Boss | High-ranking Unstained cleric boss. Bitten by a Chthonian (which only seek the infected), causing her sanity to shatter and her long-suppressed blood power to erupt. **Phase 1** (100%→30% HP): silver aura pulses every 60 ticks dealing 3 magic damage + Weakness to blood-active (Harbinger) players within 6 blocks. **Phase 2** (≤30% HP): speed/attack boost, blood spear drains 250 blood per hit from blood-active players. Drops Pale Silver Ingot ×2 + Hematic Iron Scrap ×3 on death. Boss bar: purple NOTCHED_10. `AnnettaKnowlesEntity` in `entity/boss/annetta/`. Model/texture/animations still WIP. |
...
- **Progression Codex / Liber Sanguinum** — `HemoProgressionScreen.setupEntries()` is still commented out (Java renderer WIP). However, the HutosLib JSON book framework is wired: the `sanctumsanguinium` book folder now has normal lore/mechanics chapters, and the `liberimmaculatus` book folder has 4 chapters (intro, sacred_tools, our_lady, the_path) with 12 pages covering the full Unstained path. The first Field Notes / memo slice is implemented: `memo_capture:<id>` dialogue events write memo IDs into Field Notes, and the Dictation Table dictates those IDs into the player's `LiberKnowledge` attachment rather than into the Liber item stack. `LiberKnowledge` stores `KnownMemos`, `UnlockedLiberEntries`, and per-entry discovery sources, syncs to clients with `PacketSyncLiberKnowledge`, and can be granted through `LiberKnowledgeHelper` by memos, advancements, rites, item pickups, degree changes, dialogue, or other future triggers. `LiberEntryDefinitions` is the central code-side page map: it lists visible Liber Sanguinum entries and maps initial rites, Harbinger degree advancements, Unstained milestones, selected item pickups, and `liber_unlock:<entry_id>` dialogue events to normal book page IDs. The Liber item now behaves like a personal viewer/key: borrowed books show the reader's own unlocked pages, not the owner's. Legacy stack data is migrated into the player attachment when an old Liber is used or placed for dictation. `MemoBookFilter` treats `LiberEntryDefinitions` as the source of visible Liber Sanguinum pages; pages not mapped by a definition remain hidden, and chapters with zero unlocked pages are omitted from the book entirely. The current test memo (`first_rite_notes`) unlocks a page inside `the_hematic_order`, not a standalone Field Notes chapter. Remaining WIP: re-enable `setupEntries()` if that older renderer is revived, move entry definitions to data-driven JSON if desired, and author more entry definitions across existing chapters.
...
- **~~Visceral Organs System~~** — **RESOLVED:** All 5 organ effects are fully implemented in `VisceralOrgansEvents`: Spleen (+1000 max blood per level), Liver (remove Poison at level 2+, Wither at level 3+), Lungs (Water Breathing while underwater), Kidneys (Regeneration), Heart (Damage Resistance + blood drain cost per 2s). See §13.8.
