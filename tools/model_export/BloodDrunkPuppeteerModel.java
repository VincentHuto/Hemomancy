...
public class BloodDrunkPuppeteerModel<T extends BloodDrunkPuppeteerEntity>  extends EntityModel<T> {
	...
	public void setupAnim(BloodDrunkPuppeteerEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.right_arm.yRot = this.left_arm.yRot = 0.0F;
		this.right_leg2.xRot = this.left_leg2.xRot = 0.0F;
		this.right_leg2.yRot = this.left_leg2.yRot = 0.0F;
		this.right_leg2.zRot = this.left_leg2.zRot = 0.0F;
		if (entity.getCarouselHorse() >= 0) {
			this.head.xRot = 0.15F;
			this.head.yRot = 0.0F;
			this.body.xRot = 0.12F;
			this.right_arm.xRot = this.left_arm.xRot = -1.35F;
			this.right_arm.zRot = -0.4F;
			this.left_arm.zRot = 0.4F;
			this.right_leg.xRot = this.left_leg.xRot = -1.25F;
			this.right_leg.yRot = -0.22F;
			this.left_leg.yRot = 0.22F;
			this.right_leg.zRot = this.left_leg.zRot = 0.0F;
			this.right_leg2.xRot = this.left_leg2.xRot = 1.4F;
			return;
		}
		float f = 1.0F;
