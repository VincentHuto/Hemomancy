...
async function testChalybeateSnailScleritePartsAreExplicit() {
  const outputDir = await mkdtemp(path.join(tmpdir(), "hemomancy-bbmodel-"));
  const outputFile = path.join(outputDir, "ChalybeateSnailModel.bbmodel");

  try {
    const javaSource = await readFile(
      path.join(ROOT, "src/main/java/com/vincenthuto/hemomancy/client/model/entity/mob/aquatic/ChalybeateSnailModel.java"),
      "utf8"
    );
    assert.doesNotMatch(javaSource, /for\s*\(\s*int\s+i\s*=\s*-4\s*;/,
      "sclerite plates should be declared explicitly instead of generated by a loop");
    ...
    for (const side of ["front", "left", "right"]) {
      for (const position of [-4, -2, 0, 2, 4]) {
        assert.ok(groupNames.includes(`${side}_plate_${position}`),
          `explicit ${side} plate at ${position} should be exported`);
      }
    }
  } finally {
    await rm(outputDir, { recursive: true, force: true });
