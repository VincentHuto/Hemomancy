...
Warden and Sculk Catalyst specimens identify as the separate **Ahaematic Colloid** item, with an **Incertae** readout and a gritty black/cyan slide. They show no blood tendency or enzyme response. Nearby vibration and the Severed Record can produce distinct visible responses; these specimens never count as clinical blood-source evidence. See [Antecedent Inquiry](Antecedent-Inquiry.md).
...
Craft it with two copper ingots, an iron ingot, a glass pane and leather. At Degree 1, show the Alchemist a filled creature vial to learn its recipe. Return after your first successful examination for the injection lesson. See [Clinical Blood Tools](Clinical-Blood-Tools.md). The vial rack is not an examination container.

Identification reveals the vial's current tendencies, biological properties, and injection prediction in its tooltip. After learning Borrowed Physiology from the Alchemist, unexamined blood remains injectable; before that lesson, injection is unavailable. Identification changes neither potency nor other vials of the same species, and grants no bestiary surrender rewards.
...
These textures reload through the ordinary resource manager; the overlay retains no composition cache. Tendency tint comes from the existing enum palette. Built-in biological motifs use `hemomancy:blood_properties/*` and `hemomancy:fungal`. Additional namespaced properties in entity blood profiles synchronize independently of injection definitions and display as stable IDs when no translation exists. Unrecognized motifs use neutral motes.

Editable item source: `assets/hemomancy/models/item/bbmodel/hematic_microscope.bbmodel`. The model generator is `tools/oneoff/hematic_microscope.py`; use `--model-only` to rebuild the socket, editable Blockbench model, and mirrored viewing model without changing any item or GUI textures. Without that option it preserves existing GUI PNGs unless explicitly invoked with `--reset-gui-textures`. The original focusing loop can be regenerated with `tools/audio/author_microscope_loop.py`; its sound event is `hemomancy:item.microscope.examine_loop`.
