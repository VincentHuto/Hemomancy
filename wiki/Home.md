# Hemomancy Wiki

> This wiki is a downstream player-facing presentation of `docs/LORE_REFERENCE.md` and `docs/HEMOMANCY_REFERENCE.md`. When wording differs, the docs are canonical and this wiki must be corrected.

![Hemomancy Banner](https://img.shields.io/badge/Minecraft-1.21.1-brightgreen) ![NeoForge](https://img.shields.io/badge/NeoForge-21.1.x-orange) ![Version](https://img.shields.io/badge/Version-6.0.1-blue)

Welcome to the official wiki for **Hemomancy**, a blood magic mod for Minecraft that explores the moral ambiguity of power drawn from the carnal fluid flowing through your veins.

> *"Channel the carnal fluid flowing through your veins, don't let it go to waste. Use it to crush your enemies, or boost yourself! After all, it's just sitting there!"* — Huto

---

## What is Hemomancy?

Hemomancy is a comprehensive blood magic and fungal horror mod for Minecraft 1.21.1 (NeoForge). Unlike typical magic mods that focus on accumulating more power, Hemomancy emphasizes the **quality** of blood manipulation over quantity. The mod presents a morally gray world where blood magic is neither inherently good nor evil — it is a tool, a heritage, and a burden.

### Two Paths, One Choice

Players face a fundamental choice:

- **The Harbinger Path** — Embrace blood magic as a sacred inheritance. Join the Hematic Order, ascend through seven degrees of initiation, and unlock powerful blood manipulations. But know that each step brings you closer to something ancient and incomprehensible.

- **The Unstained Path** — Reject blood magic as infection. Purify yourself through brutal rituals using hemolytic solution and copper, guided by Our Lady of Still Waters. Become biologically immortal, but at a cost few understand.

**These paths are mutually exclusive.** Choose wisely.

---

## Quick Navigation

### For Players
- **[[Getting Started]]** — Installation, first steps, and the Mortal Display
- **[[Public Alpha Readiness]]** - Current alpha status, known limitations, and tester path
- **[[Harbinger Path]]** — Blood magic progression, degrees, and manipulations
- **[[Unstained Path]]** — Purification mechanics and guardian powers
- **[[Blood Systems]]** — Understanding manipulations, tendencies, and the vascular system
- **[[World Content]]** — Biomes, structures, mobs, and items to discover
- **[[Advanced Mechanics]]** — Morphlings, puppeteering, cardinal rites, and post-alpha Drudge WIP notes

### For Lore Enthusiasts
- **[[Lore and Story]]** — The Fungal Entity, factions, characters, and moral themes
- **[[Factions]]** — The Hematic Order, The Unstained, and Our Lady of Still Waters
- **[[The Qliphoth]]** — The Tree of Death and fungal communion
- **[[Saints and Encounters]]** — Hemorath, the Chain Saint, Annetta Knowles, and trial chambers

### For Modpack Creators
- **[[Mod Compatibility]]** — Required dependencies, JEI support, and dormant MnA/Curios notes
- **[[Configuration]]** — Server and client config options

### For Developers
- **[[Developer Reference]]** — Links to comprehensive technical documentation
- **[[Contributing]]** — How to contribute to the mod

---

## Key Features

### Blood Manipulation System
Manipulations are organized into **eight tendencies** (blood qualities), each with unique powers:
- **Animus** (Life) — Healing, regeneration, vitality
- **Flammeus** (Fire) — Combustion and heat
- **Ductilis** (Lightning/Nerves) — speed, nervous energy, and electrical impulse
- **Lux** (Light) — Radiance and purification
- **Mortem** (Death) — Decay and withering
- **Congeatio** (Ice) — Freezing and preservation
- **Ferric** (Iron) — barbs, solidity, permanence, and magnetic force
- **Tenebris** (Shadow) — Darkness and concealment

### Progressive Degrees
Harbingers advance through **eight degrees** of the Hematic Order:
1. **Neophyte**
2. **Votary**
3. **Initiate**
4. **Adept**
5. **Illuminatus**
6. **Sanctified**
7. **Archon**
8. **Apotheos** — reached only through the post-projection Rite of Apotheosis

### Living World
- **Fungal Biomes** — Explore the eerie Sporecrown Thicket, Hyphal Spires, and Fungal Gardens
- **Dynamic Structures** — Discover Harbinger Outposts, Mausoleums, and hidden trial chambers
- **Blood Moons** — Endure a fungal surge while the Pale Lady mounts a costly containment response
- **Unique Mobs** — Face Sanguine Thralls, Morphlings, Blood Constructs, and worse

### Deep Lore
Hemomancy features a rich narrative that challenges typical fantasy tropes:
- The Fungal Entity is an incomprehensible 4D organism, not a simple demon
- Harbingers are a found family, not villains
- The Unstained's methods are brutal, not heroic
- Common folk view blood magic like vultures — unsettling but not evil
- Every choice has consequences

---

## Important Notes

### Moral Ambiguity
This mod intentionally avoids "good vs evil" framing. Neither path is objectively correct. Both have costs. The Harbingers are taboo but not wicked; the Unstained are pure but not righteous. Approach the mod with this nuance in mind.

### Opt-In Gameplay
**Hemomancy is completely opt-in.** The mod's mechanics only activate when you choose to interact with the Mortal Display. While fungal biomes and structures exist in the world, natural infection is not a player mechanic unless you deliberately embrace it.

### Performance & Compatibility
- Requires **NeoForge 21.1.x** (not legacy Forge)
- Minecraft version **1.21.1**
- Java **21** required
- JEI support is alpha-ready through the current local NeoForge JEI jar
- Mana and Artifice and Curios integrations are dormant on the NeoForge 1.21.1 branch until compatible dependencies are available
- Depends on **HutosLib** (required library mod)

---

## Getting Help

- **Issues & Bug Reports:** [GitHub Issues](https://github.com/VincentHuto/Hemomancy/issues)
- **Discussions:** [GitHub Discussions](https://github.com/VincentHuto/Hemomancy/discussions)
- **Source Code:** [GitHub Repository](https://github.com/VincentHuto/Hemomancy)

---

## Credits

**Hemomancy** is developed by **VincentHuto** with contributions from the community.

**Required Dependencies:**
- [HutosLib](https://github.com/VincentHuto/HutosLib) — Core library providing shared utilities
- [GeckoLib](https://github.com/bernie-g/geckolib) — Entity animations
- [TerraBlender](https://github.com/Glitchfiend/TerraBlender) — Biome integration

**Optional Integrations:**
- [Just Enough Items (JEI)](https://www.curseforge.com/minecraft/mc-mods/jei) — Recipe viewing, alpha-ready in the current branch
- [Mana and Artifice](https://www.curseforge.com/minecraft/mc-mods/mana-and-artifice) — Dormant design/source target until a compatible NeoForge 1.21.1 build exists
- [Curios API](https://www.curseforge.com/minecraft/mc-mods/curios) — Dormant design/source target until a compatible NeoForge 1.21.1 build exists

---

## License

All Rights Reserved © VincentHuto

---

*Welcome to a world where blood is memory, magic is infection, and the line between hero and monster has never been thinner.*
